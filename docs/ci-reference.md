...
The verify step exists because `playwright install` decides "already installed" from the mere **existence** of the revision directory. An interrupted download — a killed job, a full disk, a truncated `actions/cache` restore — leaves that directory in place with a fraction of its files and a stub launcher binary, and every later `playwright install` is a silent no-op. Observed on a dev machine: `chromium-1208` with 39 files / 428 KB and a 52 KB stub binary, which `npx playwright install chromium` would not repair. A leg in that state does not report "my browser is broken"; it reports whatever the specs happen to fail on, which reads like a code defect.

So the check actually launches the browser and renders a page in it — the only test that covers the binary the specs will use, since a headless `chromium.launch()` resolves to the separate `chromium_headless_shell` download rather than the path `executablePath()` names. On failure it annotates the job with an explicit "this is an ENVIRONMENT failure, do not debug the specs" error, and `--repair` deletes the pinned revision (both downloads) and re-fetches it, so the runner heals without hands-on access. It also runs on `js-live`, `js-windows` (whose cache is restored by `actions/cache` — same truncation hazard) and `update-visual-baselines` (a golden image generated by a half-installed browser would be committed and then enforced).

Both scripts are covered by `tests/python/test_ci_workspace_hygiene.py` (behaviour against a synthetic git repo, plus assertions that `ci.yml` still wires them in and still keeps the macos legs advisory) and `tests/js/ci-playwright-check.test.js` (the path parsing and size threshold), because a mistake in CI-only glue is otherwise only discoverable by a full CI round trip on an advisory leg.
