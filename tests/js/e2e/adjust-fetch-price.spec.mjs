...
const INVENTORY = [
  ...
  // Ledger row exists (real purchase history) but its lastPoQty is unparseable/blank —
  // has_purchase_history must still report true so the Delete button stays hidden,
  // even though get_last_po_quantity (a different signal) returns null for it.
  { section: "Passives - Capacitors > MLCC",
    lcsc: "C-BLANKQTY", digikey: "", pololu: "", mouser: "",
