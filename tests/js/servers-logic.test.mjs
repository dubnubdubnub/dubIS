...
describe('classifyProbe', () => {
  ...
  it('calls a 200 without the dubIS payload foreign, not live', () => {
    ...
    expect(c.detail).toMatch(/not dubIS/);
  });

  it('reports an error status and a network failure distinctly', () => {
    expect(classifyProbe({ ok: false, status: 502 }).detail).toBe('HTTP 502');
    expect(classifyProbe({ ok: false, error: 'timed out' }).detail).toMatch(/timed out/);
