...
- **Header additions can push the panels off-screen:** `.header` is `flex-wrap: wrap`, so a new control adds a whole ~28px row at narrow widths and shifts every panel down — enough to shove the Import button off an 800×600 viewport. `resize-visibility.spec.mjs` catches it; the fix is a `max-width: 1199px` media query hiding the control below the app's documented 1200×700 minimum (see `css/components/ui-zoom.css`), not loosening that spec.
