...
- **Server picker**: Preferences keeps a roster of servers in `data/preferences.json`'s `servers` (`[{id, name, url}]`) and the app connects to whichever one `server_url` names — the roster is only the menu, so `remote_mode.py`, `app_restart.py` and `tools/dubis-cli` still read the single `server_url` key and needed no change. `js/servers-logic.js` holds every decision (validation, the add/edit/remove transforms, and what each row's dot may claim), `js/server-probe.js` does the cross-origin `GET /v1/health`, `js/server-list.js` renders the rows and polls only while the modal is open. A dot means *reachable from this window*, which is why the probe runs in the browser and not on the server; two rows can't be probed at all and must read as unknown rather than red — the Local row while this window is remote (nothing is listening yet; selecting Local *starts* a server), and an `http://` target on an `https` page (blocked as mixed content before it reaches the network). `/v1/health` carries `Access-Control-Allow-Origin: *` so the probe can read it; no other route does, and `tests/python/server/test_health_cors.py` fails if that spreads.
