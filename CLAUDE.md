...
```
Missing credentials cause a test failure with an actionable message — they do not skip.

The Mouser API key is optional **for the app** — without one, `mouser_client` renders the product page in a hidden webview (`browser_page.py`) and parses `table.pricing-table`, which is where the per-carrier ladders and the MouseReel fee come from. It is not optional **here**: pytest has no GUI event loop, so `browser_page.available()` is false and the keyless path falls through to the urllib scrape that Mouser blocks. Keep the key configured or `pytest -m live` proves nothing about Mouser. If live runs reveal upstream API drift, refresh the committed fixtures and commit the result:
```bash
...
- **Container feature gaps**: the container image is desktop-feature-limited by design — no DigiKey WebView2/CDP scraping, no keyless Mouser fetching (`browser_page.available()` is false without a GUI loop, so it falls back to the urllib scrape Mouser blocks; configure a Mouser API key to price parts in-cluster), no OS file dialogs, no OCR (tesseract not installed). Those API methods already fail with typed errors when their dependency is unavailable; this is documented, not a bug to fix in-container.
