...
- **`data/` means two different directories:** on the desktop the static root *is* the repo, so `<img src="data/lcsc-icon.ico">` and the backend's data dir are the same folder and everything resolves. dubis-server serves the image root (/app) and keeps user data on a PVC (/data), so a *frontend* asset under `data/` (the four distributor icons, the browser-tab icon) only resolves if the Dockerfile copies it into the image — `tests/python/test_container_assets.py` fails when a new one is referenced and not copied. Anything that genuinely lives in the *user's* data dir can never be a static URL at all: a fetched vendor favicon (`sources/favicons/<hash>.png`) has to travel as the `favicon_data_uri` that `/v1/vendors` already returns, which is what `vendorIconFor` in `js/ui-helpers.js` prefers. Both halves look flawless on the desktop app and 404 on every remote client. Same trap, server side: `server/routes/openpnp.py` reads its family table from the image root, not the data dir, and answers a missing file with `{}`, so an uncopied asset degrades silently instead of erroring.
