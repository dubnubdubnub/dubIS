...
class DistributorFacade:
    ...
    def create_jlc_pairing(self) -> dict[str, Any]:
        return self._api._distributors.create_jlc_pairing()

    def receive_jlc_session(
        self, nonce: str, account: str = "", cookies: Any = None, label: str = "",
    ) -> dict[str, Any]:
        return self._api._distributors.receive_jlc_session(nonce, account, cookies, label)

    def list_jlc_sessions(self) -> dict[str, Any]:
        return self._api._distributors.list_jlc_sessions()

    def revoke_jlc_session(self, account: str) -> dict[str, Any]:
        return self._api._distributors.revoke_jlc_session(account)

    def fetch_jlc_library(self, account: str = "") -> dict[str, Any]:
        return self._api._distributors.fetch_jlc_library(account)
